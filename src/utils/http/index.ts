...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    if (axios.isCancel(error)) {
      ...
      const errorMessage =
        error.response?.data.msg === '' ? error.response?.data.data : error.response?.data.msg
      ElMessage.error(
        errorMessage
...
const api = {
  ...
  uploadPricture<T>(files: File[], dir: string, token: string) {
    const formData = new FormData()
    files.forEach((file) => {
      formData.append('files', file)
    })
    return axios.post(import.meta.env.VITE_API_URL + '/biz/v1/oss/upload?dir=' + dir, formData, {
      headers: {
